// scripts/test-security-fixes.cjs - Test the security fixes we made (CommonJS version)
const fs = require('fs');
...
function testContractSecurity() {
  ...
  console.log('✓ Public decrypt calls for verification:', publicDecryptUsages.length >= 1); // In finalizeWithdraw
  
  if (hasReentrantFlag && hasNonReentrantModifier && hasPublicDecryptUsed && publicDecryptUsages.length >= 1) {
